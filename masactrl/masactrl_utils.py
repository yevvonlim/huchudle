...
def register_attention_editor_dit(model, editor: AttentionBase):
    """
    Register a attention editor to Stable Diffusion model, refer from [Prompt-to-Prompt]
    """
    def ca_forward(self, place_in_unet):
    ...
    def sa_forward(self):
        def forward(x):
            ...
            out = editor(q, k, v, sim, attn, 'False', '', self.num_heads, scale=self.scale)
    ...
    def register_editor(net, count, place_in_unet):
        for name, subnet in net.named_children():
            if net.__class__.__name__ == 'CrossAttention':
                net.forward = ca_forward(net, place_in_unet)
                return count + 1
            elif net.__class__.__name__ == 'Attention':
                net.forward = sa_forward(net)
                return count + 1
            elif hasattr(net, 'children'):
                count = register_editor(subnet, count, place_in_unet)
        return count
