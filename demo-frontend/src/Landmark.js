...
const Landmark = forwardRef((props, ref) => {
  const landmarks = {
  ...
  const [initialPositions, setInitialPositions] = useState(circles);
  ...
  const savePositions = () => {
    setFinalPositions(circles);
  };

  const sendPositionsToServer = async () => {
    try {
      await axios.post('https://your-server-endpoint.com/api/positions', {
        initialPositions,
        finalPositions
      });
      console.log('Positions sent to server successfully');
    } catch (error) {
      console.error('Error sending positions to server:', error);
    }
  };

  const saveAndSendPositions = () => {
    savePositions();
    sendPositionsToServer();
  };

  useImperativeHandle(ref, () => ({
    saveAndSendPositions
  }));
