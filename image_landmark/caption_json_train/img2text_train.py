    ...
    try:
        with torch.no_grad():
            for idx, (img, fname) in enumerate(tqdm(dataloader)):
                ...
                prompt = ["Question: What color are this person's eyes? Answer: "] * img.shape[0]
                ...
                generated_text = [text.split("Question: What color are this person's eyes? Answer: ")[1].strip() for text in generated_text]
                generated_text = [f"This person's eye color is {text}. " for text in generated_text]
                ...
                # generated_text = processor.batch_decode(generated_ids, skip_special_tokens=True)
                # generated_text = ""
                # # prompt = ["Question: What is this person's eye color? Answer: "]*img.shape[0]
                # prompt = [f"This person eye color is {generated_text[i]}. Question: What is this person's eye color? Answer: " for i in range(img.shape[0])]
                # inputs = processor(img, text=prompt, return_tensors="pt", padding=True).to(device, torch.float16)
                # generated_ids = model.module.generate(**inputs, max_new_tokens=20)
                # generated_text = processor.batch_decode(generated_ids, skip_special_tokens=True) 
                
                prompt = [f"{generated_text[i]}. Question: What is this person's eye color? Answer: " for i in range(img.shape[0])]
                inputs = processor(img, text=prompt, return_tensors="pt", padding=True).to(device, torch.float16)
                generated_ids = model.module.generate(**inputs, max_new_tokens=20)
                only_generated_text = processor.batch_decode(generated_ids, skip_special_tokens=True) 
                generated_text = "This person's eye color is " + only_generated_text

            
                prompt = [f"{prompt[i]}{generated_text[i]}, Question: What color are this person's eyes? Answer: " for i in range(img.shape[0])]
                inputs = processor(img, text=prompt, return_tensors="pt", padding=True).to(device, torch.float16)
            # with torch.autocast('cuda'):
                ...
                prompt = [f"This person skin color is {generated_text[i]}. Question: What color is this person's skin? Answer: " for i in range(img.shape[0])]
                inputs = processor(img, text=prompt, return_tensors="pt", padding=True).to(device, torch.float16)
            # with torch.autocast('cuda'):
                generated_ids = model.module.generate(**inputs, max_new_tokens=5)
                generated_text = processor.batch_decode(generated_ids, skip_special_tokens=True)
            ...
            # with torch.autocast('cuda'):
                ...
                if (idx%50==0):
                    with open(f'/workspace/image_landmark/caption_json/0_output_cropped_json/caption_result{rank}.json', 'w') as fp:
                        json.dump(result, fp)
...
if __name__ == "__main__":
    ...
    dist.init_process_group(backend='nccl')
                            ...
                            # rank=rank)
    dataset = ImgCapDataset('/workspace/image_landmark/cropped_image/0_output_cropped_img')
