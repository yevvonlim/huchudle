...
# https://github.com/CompVis/stable-diffusion/blob/main/ldm/modules/attention.py#L152C1-L193C32
# @yewon
def exists(val):
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context=None, mask=None):
        ...
        out = F.scaled_dot_product_attention(q, k, v, scale=self.scale, dropout_p=self.dropout)
...
class TextEmbedder(nn.Module):
    def __init__(self, text_encoder, tokenizer, dropout_prob):
        super().__init__()
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.text_embedder = TextEmbedder(self.text_encoder, self.tokenizer, class_dropout_prob)
    ...
    def forward(self, x, t, y, landmark, token=None, text_emb=None):
        ...
        """
        if self.training and torch.rand(1) < self.text_embedder.dropout_prob:
            landmark = torch.zeros_like(landmark).to(landmark.device)
...
# Ironically this is our only option lol
...
def count_attn_layers(model):
    ret  = 0
    for name, module in model.named_modules():
        if name == 'text_encoder' or name == 'tokenizer' or name == 'text_embedder':
            continue 
        if isinstance(module, CrossAttention) or isinstance(module, Attention):
            ret += 1
    return ret
