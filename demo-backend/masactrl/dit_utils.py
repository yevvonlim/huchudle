...
from huchu_models import DiT_models
import argparse
from masactrl.masactrl import MutualSelfAttentionControl
from masactrl.masactrl_utils import register_attention_editor_dit
from typing import List
from prompt_to_prompt.ptp_attn_class import AttentionReplace,AttentionRefine,AttentionReweight,AttentionControlEdit,LocalBlend,get_equalizer,make_controller
...
class FACEPipeline:
    ...
    def invert(self, image, landmark_img, prompt, return_intermediate=False, progress=True):
        ...
        if self.inversion == "exceptional":
            ...
            model_kwargs = dict(y=prompt, landmark=landmark_img)
        
        print("x shape",x.shape)
        print("self.inversion",self.inversion)
    ...
    def sample(self, prompt:List[str], landmark_img, latent=None, save_img=True, progress=True):
        ...
        if self.inversion == "cfg":
            ...
            y = prompt + ["",]
            model_kwargs = dict(y=y, landmark=landmark_img, cfg_scale=5.)
            with torch.no_grad():
    ...
    def edit_prompt2prompt(self, og_prompt,new_prompt, orig_landmark_img, latent, cfg_scale=5., save_img=True,controller= None ):
        z = torch.cat([latent, latent], 0).to(self.device)
        print(f"latent shape: {z.shape}")
        prompts= [og_prompt, new_prompt]

        ## example 
        # params
        cross_replace_steps = {'default_': .8,}
        self_replace_steps = .5
        blend_word = ((('black',), ("blue",))) # for local edit. If it is not local yet - use only the source object: blend_word = ((('cat',), ("cat",))).
        eq_params = {"words": ("hair",), "values": (2,)} # amplify attention to the word "tiger" by *2 
        ##
        # controller = make_controller(prompts, True, cross_replace_steps, self_replace_steps)
        # controller = make_controller(prompts, True, cross_replace_steps, self_replace_steps, blend_word, eq_params)
        controller = AttentionReplace(prompts, NUM_DIFFUSION_STEPS, cross_replace_steps={"default_": 1., "lion": .4},self_replace_steps=0.4)

        # replace attention
        register_attention_control(self.model, controller)

        y = [new_prompt, og_prompt]
        landmark_img = torch.cat([orig_landmark_img, orig_landmark_img], 0).to(self.device) # change both to og image
        model_kwargs = dict(y=y, landmark=landmark_img, cfg_scale=cfg_scale)
        with torch.no_grad():
            samples = self.diffusion.ddim_sample_loop( 
                self.model.forward_with_cfg,
        ...
        samples = self.vae.decode(samples / 0.18215).sample
        if save_img:
            save_image(samples, f"/workspace/project-root/demo-backend/temp/edit_{new_prompt}.png", nrow=4, normalize=True, value_range=(-1, 1))
